...
<link rel="stylesheet" type="text/css" href="../css/lifeFood.css"/>
		...
		<script type="text/javascript">
		   ...
		   document.getElementsByClassName('headerNavInner')[0].getElementsByTagName('a')[3].className='active_nav';
			var url='../php/getData/getCommodityData.php?table=life_articles&page=0&category=232';
			...
			var vm=new Vue({
				...
				methods:{
					...
					getData:function(event,index){
						...
						this.active_sort=0;
						var url='../php/getData/getCommodityData.php?table=life_articles&page=0&category='+event.target.id;
						this.$http.get(url).then(function(res){
							vm.cmd_list=JSON.parse(res.bodyText)[0];
							var page=Math.floor(JSON.parse(res.bodyText)[2][0]['count(id)']/12) ;
							$arr=[];
					...
					selectPage:function (i){
						...
						this.active=i;
						if(this.active_sort){
							var url='../php/getData/getCommodityData.php?table=life_articles&page='+i+'&category='+this.category+'&sort='+this.sortId+'&top=0';
						}else{
							var url='../php/getData/getCommodityData.php?table=life_articles&page='+i+'&category='+this.category;
						}
					...
					selectPage2:function (num){
						...
						if(this.active_sort){
							var url='../php/getData/getCommodityData.php?table=life_articles&page='+this.nowPage+'&category='+this.category+'&sort='+this.sortId+'&top=0';
						}else{
							var url='../php/getData/getCommodityData.php?table=life_articles&page='+this.nowPage+'&category='+this.category;
						}
					...
					sorts:function(i,item,top){
						...
						if(top==1){
							item.flag=top;
							var url='../php/getData/getCommodityData.php?table=life_articles&page='+(this.nowPage)+'&category='+this.category+'&sort='+item.id+'&top=1';
						}else if(top==2){
							item.flag=top;
							var url='../php/getData/getCommodityData.php?table=life_articles&page='+(this.nowPage)+'&category='+this.category+'&sort='+item.id+'&top=0';
						}else{
							if(i==0){
								var url='../php/getData/getCommodityData.php?table=life_articles&page='+(this.nowPage)+'&category='+this.category;
								
							}else{
								var url='../php/getData/getCommodityData.php?table=life_articles&page='+(this.nowPage)+'&category='+this.category+'&sort='+item.id;
					...
					to_deatil:function(item){
						location.href='shipingxiangqing.php?category=life_articles&id='+item.id;
					}
				}
			})
		</script>
